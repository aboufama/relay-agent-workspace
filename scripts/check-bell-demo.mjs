...
for (const [agentId, ids] of Object.entries(expected)) {
  ...
  assert.deepEqual([...new Set(passages.map(passage => passage.documentId))].sort((a, b) => a < b ? -1 : a > b ? 1 : 0), ids.map(id => `bell-${id}`).sort((a, b) => a < b ? -1 : a > b ? 1 : 0), `${agentId}: exact source access`);
