...
function MemberClearance({member}: {member?: WorkspaceMember}) {
  return member?.kind === 'agent' ? <span className="member-clearance">({member.accessLevel.toLowerCase()})</span> : null;
}
                ...
                >
                  ...
                  <div className="message-body">
                    <div className="message-meta">
                      <button className="member-name-button" onClick={()=>{const member=resolveMember(members,m.memberId,m.name);if(member)openMember(member)}}>{members.find(person=>person.id===m.memberId)?.name ?? m.name}</button><MemberClearance member={resolveMember(members,m.memberId,m.name)} />
                      <time>{m.time}</time>
                    </div>
