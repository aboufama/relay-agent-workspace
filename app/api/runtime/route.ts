...
export async function GET() {
  ...
  const [inference, embedding, rerank, gateway] = await Promise.all([
  ...
  const nodes = [
    { id: 'spark-inference', name: 'DGX Spark · inference', kind: 'inference', status: inferenceReady ? 'inference-healthy' : inference.up ? 'model-loading' : 'unreachable', data: { models: inference.models, url: e.BUZZ_VLLM_URL ?? null } },
    { id: 'spark-embedding', name: 'DGX Spark · embeddings', kind: 'embedding', status: embedding.up ? 'inference-healthy' : 'unreachable', data: { models: embedding.models } },
    { id: 'spark-rerank', name: 'DGX Spark · reranker', kind: 'rerank', status: rerank.up ? 'inference-healthy' : 'unreachable', data: { models: rerank.models } },
  ...
  return ok({ homes: [{ id: 'lab', name: 'DGX Spark', connected, model }], nodes: rows.results.map(mapNode) });
}
