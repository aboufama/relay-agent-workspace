...
export async function POST(request: Request) {
  ...
  const available = rows.results.map(mapMember).filter((a: MemberRecord) => !a.data.paused);
  const explicit = available.filter(a => room === `dm:${a.id}` || scopeRoom === `dm:${a.id}` || mentioned(text, a.name));
  const agents = mode === 'deep' && !explicit.length && !scopeRoom.startsWith('dm:')
    ? available.filter(a => canUseContextRoom(a, scopeRoom)) : explicit;
  ...
  const stmts = [e.DB.prepare('INSERT INTO messages(id, room, member_id, name, body, created_at, state, client_id) VALUES (?, ?, ?, ?, ?, ?, NULL, ?)').bind(triggerId, room, memberId, (typeof member?.name === 'string' && member.name ? member.name : 'You'), text, ts, clientId)];
