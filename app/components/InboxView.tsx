...
const initialItems: Item[] = [
  {
    ...
    channel: 'launch-room',
    time: '8m',
    text: 'A customer update draft is ready for review.',
  },
  {
    id: 'marcus-mention',
    authorId: 'marcus',
    ...
    channel: 'engineering',
    time: '18m',
    text: 'Can you confirm the rollout owner?',
  },
];
type Props = { reviewDraft: () => void; openRoom: (name: string) => void };
export function InboxView({ reviewDraft, openRoom }: Props) {
  ...
  const [focusedId, setFocusedId] = useState(initialItems[0].id);
  const rowRefs = useRef(new Map<string, HTMLButtonElement>());
  const detailHeading = useRef<HTMLHeadingElement>(null);
  const unreadCount = initialItems.filter(
    (item) => !read.includes(item.id),
  ).length;
  const author = (item: Item) =>
    members.find((member) => member.id === item.authorId);
  const authorName = (item: Item) =>
    author(item)?.name || (item.authorId === 'nova' ? 'Nova' : 'Marcus Reed');
  const title = (item: Item) =>
    item.kind === 'review'
      ? `${authorName(item)} prepared a customer update draft`
      : `${authorName(item)} mentioned you`;
  const visible = initialItems.filter(
    (item) =>
  ...
  const selected = initialItems.find((item) => item.id === selectedId);
      ...
      <PageHeader
        ...
        action={
          <button
            ...
            onClick={() => setRead(initialItems.map((item) => item.id))}
              ...
              <article className="quality-inbox-message">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (selected.kind === 'review') reviewDraft();
                    else openRoom(selected.channel);
                  }}
                >
                  {selected.kind === 'review' ? 'Review draft' : 'Open channel'}
