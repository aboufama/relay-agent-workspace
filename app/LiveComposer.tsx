...
type Props = {value:string;onChange:(value:string)=>void;onSend:()=>void;members:readonly WorkspaceMember[];onMention:(id:string)=>void;placeholder:string};
export const LiveComposer=React.forwardRef<HTMLTextAreaElement,Props>(function LiveComposer({value,onChange,onSend,members,onMention,placeholder},forwardedRef){
 const field=React.useRef<HTMLTextAreaElement>(null);React.useImperativeHandle(forwardedRef,()=>field.current!);
 ...
 const query=match?.[1]??'';
 const options=match&&!dismissed?members.filter(m=>m.id!=='you'&&m.name.toLowerCase().includes(query.toLowerCase())).slice(0,8):[];
 const listId=React.useId();
 function choose(member:WorkspaceMember){const start=prefix.lastIndexOf('@');const inserted=`@${member.name} `;onChange(value.slice(0,start)+inserted+value.slice(caret));onMention(member.id);setDismissed(true);requestAnimationFrame(()=>{field.current?.focus();field.current?.setSelectionRange(start+inserted.length,start+inserted.length);setCaret(start+inserted.length)});}
 return <div className="live-composer-input">
  ...
  {options.length>0&&<div id={listId} role="listbox" aria-label="Mention a member" className="mention-picker">{options.map((member,index)=><button type="button" role="option" aria-selected={index===active} id={`${listId}-${index}`} key={member.id} className={index===active?'active':''} onMouseDown={e=>e.preventDefault()} onClick={()=>choose(member)}>{member.kind==='agent'?<AgentAvatar character={member.character} label={member.name} size={26}/>:<span className={`tiny-avatar ${member.tone??''}`}>{member.initials}</span>}<span>{member.name}<small>{member.kind==='agent'?'Agent':'Person'}</small></span></button>)}</div>}
