...
export async function seedBell(env: BuzzEnv) {
  ...
  for (const source of demo.documents) {
    ...
    statements.push(
      env.DB.prepare("INSERT OR IGNORE INTO documents(id, name, type, size, collection, level, owner, audiences, agents, status, text_chars, chunk_count, updated_at, r2_key) VALUES (?, ?, 'text/markdown', ?, ?, ?, ?, ?, ?, 'ready', ?, 1, ?, ?)")
        .bind(id, filename, new TextEncoder().encode(text).length, collection.name, source.classification, name(source.ownerId), JSON.stringify(collection.audience), JSON.stringify(agents), text.length, `${source.updatedAt}T12:00:00.000Z`, key),
      env.DB.prepare('INSERT OR IGNORE INTO chunks(id, document_id, idx, text) VALUES (?, ?, 0, ?)').bind(`${id}:0`, id, text),
