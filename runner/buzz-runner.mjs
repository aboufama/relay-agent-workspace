...
async function api(path, init = {}) {
  const res = await fetch(`${API}${path}`, { ...init, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${TOKEN}`, ...init.headers }, signal: AbortSignal.timeout(100000) });
...
for (;;) {
  if (active < CONCURRENCY) {
    try {
      ...
      if (claim?.run) { active++; void execute(claim).catch(error => console.error(`[${claim.run.id}] execution failed:`, error)).finally(() => active--); continue; }
