...
type Props = { title: string; leadingContent?: ReactNode; titleAdornment?: ReactNode; actions?: ReactNode };
export function ChatHeader({title,leadingContent,titleAdornment,actions}:Props) {
 return <header className="pointer-events-auto relative z-30 min-w-0 shrink-0 cursor-default select-none bg-transparent px-5 py-2" data-testid="chat-header">
  <div className="flex h-9 min-w-0 items-center gap-2.5">
   <div className="min-w-0 flex-1"><div className="group/title flex min-w-0 items-center gap-[4px] overflow-hidden">
    ...
    <h1 className="min-w-0 truncate text-base font-semibold leading-6 tracking-tight" data-testid="chat-title">{title}</h1>
    {titleAdornment}
   </div></div>
