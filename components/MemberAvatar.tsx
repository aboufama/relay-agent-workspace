...
export function MemberAvatar({ member, name, initials, size = 32 }: { member?: WorkspaceMember; name?: string; initials?: string; size?: number }) {
  ...
  return <span className="member-avatar" style={{ width: size, height: size, fontSize: Math.max(10, Math.round(size * .32)) }} role="img" aria-label={member?.name ?? name ?? 'Member'}>{member?.initials ?? initials ?? '?'}</span>;
}
